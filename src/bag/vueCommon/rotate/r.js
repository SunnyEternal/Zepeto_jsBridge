...
class lottery {
    ...
    _setup () {
        ...
        for (var i = this._count() - this.options.height + 1, j = 0, len = this.options.width + this.options.height - 2; i >= len; --i, ++j) {
        ...
        for (var i = 1, len = this.options.height - 2; i <= len; ++i) {
            this.container.find('.group:eq(' + i + ') .unit:first').not('[data-index]').attr('data-index', this._count() - i);
    ...
    _enable () {
        var _this = this;
        this.container.find('.js-start').on('click', _this.beforeRoll.bind(_this));
    }
    ...
    // 抽奖之前的操作，支持用户追加操作
    beforeRoll () {
        var _this = this;console.log(_this.options);
